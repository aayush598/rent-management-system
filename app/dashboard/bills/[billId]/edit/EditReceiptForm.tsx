...
export function EditReceiptForm({ tenant, bill }: { tenant: any, bill: any }) {
  ...
  const [includeElectricity, setIncludeElectricity] = useState(parseFloat(bill.electricityAmount) > 0 || bill.electricityCurrUnit > 0);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input required type="date" value={dateStart} onChange={e => setDateStart(e.target.value)} className="w-full px-3 py-2 text-slate-900 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">End Date</label>
            <input required type="date" value={dateEnd} onChange={e => setDateEnd(e.target.value)} className="w-full px-3 py-2 text-slate-900 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
        </div>
        ...
        <div className="space-y-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
          ...
          <label className="flex items-center gap-2 text-sm text-slate-900">
            <input type="checkbox" checked={includeRent} onChange={e => setIncludeRent(e.target.checked)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
            Rent (₹{tenant.baseRent})
          </label>
          <label className="flex items-center gap-2 text-sm text-slate-900">
            <input type="checkbox" checked={includeWater} onChange={e => setIncludeWater(e.target.checked)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
            Water (₹{tenant.waterCharge})
          </label>
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-sm text-slate-900">
              <input type="checkbox" checked={includeElectricity} onChange={e => setIncludeElectricity(e.target.checked)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
              Electricity (₹10/unit)
            </label>
            {includeElectricity && (
              <div className="grid grid-cols-2 gap-2 ml-6">
                <div>
                  ...
                  <input type="number" value={prevUnit} onChange={e => setPrevUnit(parseInt(e.target.value) || 0)} className="w-full px-2 py-1 text-sm text-slate-900 bg-white border border-slate-200 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs text-slate-500 mb-1">Curr Unit</label>
                  <input type="number" value={currUnit} onChange={e => setCurrUnit(parseInt(e.target.value) || 0)} className="w-full px-2 py-1 text-sm text-slate-900 bg-white border border-slate-200 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
              </div>
            )}
          </div>
        ...
        <div>
          ...
          <input type="number" step="0.01" value={oldPendingAmount} onChange={e => setOldPendingAmount(e.target.value)} placeholder="0.00" className="w-full px-3 py-2 text-slate-900 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500" />
        ...
        <div className="flex justify-between items-center p-4 bg-indigo-50 text-indigo-900 rounded-xl font-bold">
          <span className="flex items-center gap-2"><Calculator className="w-5 h-5" /> Total Due:</span>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button type="button" onClick={autoFillFullPayment} className="text-xs font-medium text-indigo-600 bg-indigo-50 px-2 py-1 rounded hover:bg-indigo-100">Autofill Full Amount</button>
          </div>
          
          {includeRent && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Rent Paid (₹)</label>
              <input type="number" step="0.01" value={rentPaid} onChange={e => setRentPaid(e.target.value)} placeholder={`Out of ₹${rentAmountDue}`} className="w-full px-3 py-2 text-slate-900 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
          )}
          {includeWater && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Water Paid (₹)</label>
              <input type="number" step="0.01" value={waterPaid} onChange={e => setWaterPaid(e.target.value)} placeholder={`Out of ₹${waterAmountDue}`} className="w-full px-3 py-2 text-slate-900 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
          )}
          {includeElectricity && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Electricity Paid (₹)</label>
              <input type="number" step="0.01" value={electricityPaid} onChange={e => setElectricityPaid(e.target.value)} placeholder={`Out of ₹${electricityAmountDue}`} className="w-full px-3 py-2 text-slate-900 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
          )}
