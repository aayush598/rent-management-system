...
interface EditReceiptFormProps {
  tenant: { id: number; baseRent: string; waterCharge: string };
  bill: {
    id: number;
    dateStart: string;
    dateEnd: string;
    rentAmount: string;
    waterAmount: string;
    electricityPrevUnit: number;
    electricityCurrUnit: number;
    electricityAmount: string;
    oldPendingAmount: string;
    rentPaid: string;
    waterPaid: string;
    electricityPaid: string;
    customCharges?: { name: string; amount: number; paid: number }[];
  };
}

export function EditReceiptForm({ tenant, bill }: EditReceiptFormProps) {
  const router = useRouter();
  const billCustomCharges = (bill.customCharges || []).map((c) => ({
    name: c.name,
    amount: c.amount,
    paid: c.paid,
    include: c.amount > 0,
  ...
  const {
    ...
    defaultValues: {
      ...
      includeRent: parseFloat(bill.rentAmount) > 0,
      includeWater: parseFloat(bill.waterAmount) > 0,
      includeElectricity: parseFloat(bill.electricityAmount) > 0 || bill.electricityCurrUnit > 0,
      prevUnit: bill.electricityPrevUnit || 0,
      currUnit: bill.electricityCurrUnit || 0,
      oldPendingAmount: parseFloat(bill.oldPendingAmount) || 0,
      rentPaid: parseFloat(bill.rentPaid) || 0,
      waterPaid: parseFloat(bill.waterPaid) || 0,
      electricityPaid: parseFloat(bill.electricityPaid) || 0,
      customCharges: billCustomCharges,
    },
  });
                ...
                {includeElectricity && (
                  ...
                  >
                    <div>
                      ...
                      <Input id="prevUnit" type="number" {...register("prevUnit")} />
                    </div>
                    <div>
